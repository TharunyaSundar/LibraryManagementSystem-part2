...
import { makeLendingLibrary } from "../lib/lending-library.js";
...
describe("lending library", () => {
    ...
    describe("findBooks()", async () => {
        ...
        it("must find multiple results", async () => {
            ...
            const searchResult = await library.findBooks(search);
            assert(searchResult.isOk === true);
            const expected = BOOKS.filter((b) => b.title.match(/definitive/i))
        ...
        it("must find no results", async () => {
            const searchResult = await library.findBooks({
                search: "a #definitive1 ",
            });
            assert(searchResult.isOk === true);
...
function findLangBooks(books, lang) {
    return books
        .filter((b) => b.title.toLowerCase().includes(lang))
        .sort((b1, b2) => b1.title.localeCompare(b2.title));
}
const LANG_BOOKS = {
